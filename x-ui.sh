...
# check root
[[ $EUID -ne 0 ]] && LOGE "错误：请以 root 权限运行本脚本。\n" && exit 1
...
update() {
    ...
    bash <(curl -Ls https://raw.githubusercontent.com/HanK-29/ChuAng-3X-ui-ZH/main/update.sh)
    if [[ $? == 0 ]]; then
...
update_dev() {
    ...
    XUI_UPDATE_TAG="dev-latest" bash <(curl -Ls https://raw.githubusercontent.com/HanK-29/ChuAng-3X-ui-ZH/main/update.sh)
    if [[ $? == 0 ]]; then
...
installed_script_url() {
    ...
    if [[ "$ver" =~ ^[0-9]+\.[0-9]+\.[0-9]+$ ]] && curl -fsIL -o /dev/null "https://raw.githubusercontent.com/HanK-29/ChuAng-3X-ui-ZH/v${ver}/x-ui.sh"; then
        echo "https://raw.githubusercontent.com/HanK-29/ChuAng-3X-ui-ZH/v${ver}/x-ui.sh"
    else
        echo -e "${yellow}No x-ui.sh published for the installed version (${ver:-unknown}), using main${plain}" >&2
        echo "https://raw.githubusercontent.com/HanK-29/ChuAng-3X-ui-ZH/main/x-ui.sh"
    fi
}
...
legacy_version() {
    ...
    install_command="bash <(curl -Ls "https://raw.githubusercontent.com/HanK-29/ChuAng-3X-ui-ZH/v$tag_version/install.sh") v$tag_version"
...
uninstall() {
    ...
    echo -e "卸载成功。\n"
    echo "如需重新安装，请运行："
    echo -e "${green}bash <(curl -Ls https://raw.githubusercontent.com/HanK-29/ChuAng-3X-ui-ZH/main/install.sh)${plain}"
    echo ""
...
migrate_db() {
    ...
    if ! "$bin" migrate-db -h 2>&1 | grep -q -- '-dump'; then
        ...
        LOGE "当前面板不支持 migrate-db --dump/--restore，无法执行数据库迁移。"
        return 1
    fi
...
│  ${blue}x-ui migrateDB [file]${plain}      - 转换 SQLite 数据库              │
│  ${blue}x-ui pgclient [ver]${plain}        - 安装数据库备份工具               │
...
show_menu() {
    echo -e "
╔────────────────────────────────────────────────╗
│  ${green}ChuAng 面板管理菜单${plain}                           │
│  ${green}0.${plain} 退出                                       │
│────────────────────────────────────────────────│
│  ${green}1.${plain} 安装                                       │
│  ${green}2.${plain} 卸载                                       │
│────────────────────────────────────────────────│
│  ${green}3.${plain} 重置用户名与密码                           │
│  ${green}4.${plain} 重置访问路径                               │
│  ${green}5.${plain} 重置设置                                   │
│  ${green}6.${plain} 修改端口                                   │
│  ${green}7.${plain} 查看当前设置                               │
│────────────────────────────────────────────────│
│  ${green}8.${plain} 启动                                       │
│  ${green}9.${plain} 停止                                       │
│  ${green}10.${plain} 重启                                     │
│  ${green}11.${plain} 重启 Xray                                │
│  ${green}12.${plain} 查看状态                                 │
│  ${green}13.${plain} 日志管理                                 │
│────────────────────────────────────────────────│
│  ${green}14.${plain} 开启开机自启                             │
│  ${green}15.${plain} 关闭开机自启                             │
│────────────────────────────────────────────────│
│  ${green}16.${plain} SSL 证书管理                              │
│  ${green}17.${plain} Cloudflare 证书                           │
│  ${green}18.${plain} IP 数量限制                              │
│  ${green}19.${plain} 防火墙管理                               │
│  ${green}20.${plain} SSH 端口转发                             │
│  ${green}21.${plain} PostgreSQL 管理                          │
│────────────────────────────────────────────────│
│  ${green}22.${plain} 启用 BBR                                 │
│  ${green}23.${plain} 更新地理规则文件                         │
│  ${green}24.${plain} Ookla 网速测试                           │
╚────────────────────────────────────────────────╝
"
    show_status
    echo && read -rp "请输入选项 [0-24]：" num
