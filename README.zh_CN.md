...
**ChuAng** 是基于 3X-UI 修改的开源 Web 控制面板，用于管理 [Xray-core](https://github.com/XTLS/Xray-core) 服务器。

本版本提供中文安装提示，并移除了面板和管理菜单中的面板更新入口。基于 [MHSanaei/3x-ui](https://github.com/MHSanaei/3x-ui) 开发；保留原项目的 GPL-3.0 许可证及版权署名。Go 模块路径仍沿用上游，以保持现有导入和构建兼容。
...
首次上传源码后，请等待仓库 GitHub Actions 构建成功，并确认 Releases 中出现 `dev-latest` 及对应的 `.sha256` 文件，然后再运行上面的安装命令。没有稳定版标签时，安装程序会使用该仓库的 `dev-latest` 构建；没有 Release 时不会转向上游下载。安装后不提供面板更新入口。

安装过程中会生成随机的用户名、密码和访问路径。安装完成后，运行 `x-ui` 打开管理菜单，您可以在其中启动/停止服务、查看或重置登录凭据、管理 SSL 证书等。

安装程序要求发布资源附带 `.sha256` 校验和；缺失或不匹配时会中止。

项目文档见本仓库的 [`docs/`](docs/) 目录。
